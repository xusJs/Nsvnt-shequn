    ...
    <nav class="personal_nav" v-if="projectList.orUserInfo !==null ">
      ...
      <div class="nav_center">
        <div>
          ...
          <span>{{projectList.peUserInfo.named}}</span>
        </div>
      </div>
        ...
        <li class="section_item" v-for="(item,index) in projectList.baseList" :key="item+index">
          <div class="project_top">
            ...
            <div>
              ...
              <p>{{projectList.peUserInfo.named}}</p>
            </div>
            <!--   <span> + 关注</span>-->
          </div>
          <div class="project_section"
               @click="$router.push({name:'ProjectDetails',params:{ProjectId:item.id,ProjectType:'1'}})">
            <p>{{item.title}}</p>
            <div>
...
<script>
  ...
  export default {
    ...
    methods: {
      MyEnterpriseProject() {
        let $self = this
        $self.$store.dispatch('_MyPEnterpriseProject')
          .then((res) => {
            if (res.success) {
